...
2. `aaa2/engine/entities_rules.py` — determinisztikus kör LLM nélkül (`aaa entities <domain>`): JSON-LD `@type` → entitás (`engine/config/schema_types.toml`), a site brand-neve a title-ben és a H1-ben, legalább 3 oldalon azonos anchor → concept-jelölt (kimarad a betű nélküli, az önhivatkozó és a nyelvváltó link); aliasok kulcs szerint (kis-nagybetű, ékezet, kötőjel); futásonként egy `entity_runs` sor. A 006-os migráció: `entity_runs`, `llm_calls.last_error`. A három referencia-készletet a `tests/conftest.py` `reference_crawl` fixture-je futásonként egyszer játssza vissza.
