...
- **A DuckDB-séma a szerződés.** `aaa2/db/001_init.sql`. Modulok csak a sémán keresztül beszélnek. Új oszlop mehet új migrációban (`002_*.sql`), meglévő oszlop jelentése nem változik. Rétegek: motor (`engine`) → entitás-réteg (`entities`) → funkciók (`functions`); a `resolver` címkéz. A motor a crawl-táblákat írja, az entitás-réteg ezekből olvas és az entitás-táblákat tölti, a `functions` csak olvas. Az `entities` a motorból csak a `parse` publikus segédfüggvényeit (`anchor_text`, `schema_items`) importálhatja, crawl- vagy render-modult nem; a motor semmit az `entities`-ből (`tests/test_boundaries.py` őrzi, a köztes modulokon át is).
...
2. `aaa2/entities/rules.py` — determinisztikus kör LLM nélkül (`aaa entities <domain>`): JSON-LD `@type` → entitás (`entities/config/schema_types.toml`); a site neve a title-ben és a H1-ben (ha egy org neve, az org kapja, a ≤ 4 jelű rövidítése alias; különben brand); legalább 3 oldalon azonos anchor → a talált entitáshoz, vagy concept-jelölt, ha egyetlen célra mutat (kimarad a betű nélküli, ≤ 2 jelű, római szám, önhivatkozó, kezdőoldalra, nem crawlolt oldalra és más nyelvű oldalra mutató link); aliasok kulcs szerint (kis-nagybetű, ékezet, kötőjel), person-nél a kéttokenes név mindkét sorrendje; futásonként egy `entity_runs` sor. A 006-os migráció: `entity_runs`, `llm_calls.last_error`. A három referencia-készletet a `tests/conftest.py` `reference_crawl` fixture-je futásonként egyszer játssza vissza.
...
3. `aaa2/entities/llm.py` — LLM-kör (`aaa entities <domain> --llm [--provider gemini] [--limit N]`): oldalanként egy hívás a title-re, a headingekre és a main contentre. A prompt állandó, a tíz típus definíciójával; entitáslista sosem kerül bele (teszt őrzi). Fabrikáció-szűrő kódban: az evidence whitespace-normalizálva, kis-nagybetű-érzéketlenül benne kell legyen a main contentben, egy headingben vagy a title-ben, különben eldobva és számolva (`llm_calls.fabricated_count`, `entity_runs.fabricated`); 3–15 szó. Összevonás a szabály-entitásokkal kulcs és alias szerint (person-nél a névsorrend is), a meglévő entitás típusa és forrása marad. Az LLM típusa csak szavazat: minden elfogadott sor egy szavazat (`entities.type_votes`, halmozódik), `type_suggested` a legtöbb szavazatot kapott típus; a típust csak az M2/4 KG-validálása állíthatja át. A 007-es migráció: `llm_calls.fabricated_count`, `entity_runs.model`, `cost_usd`, `fabricated`; a 008-as: `entities.type_suggested`, `type_votes`. Fejlesztés és teszt Gemini 3.8 Flash-sel; a modellek párhuzamos futása az M2/5.
4. `aaa2/entities/validate.py` — validálás (`aaa validate <domain>`): Google KG Search API a legacy ötkategóriás osztályozójával (high / medium / stub / ambiguous / no_match) név és típus szerint: pontos név- vagy alias-egyezés az `alias_key` szerint, és a KG-típus (`entities/config/kg_types.toml`) az entitás típusa vagy a `type_suggested` (conceptnél a csak "Thing" is) → `kg_status`, `kg_id`, `kg_type`. Más típusú, azonos nevű találat: no_match, a típusa a `kg_type`-ban, `kg_type_mismatch` jelöli. A típus csak akkor áll át, ha a KG-típus = `type_suggested` és a találat high / medium (`type_changed_from` a régi). Wikipedia az entitás nyelvén, majd angolul: a keresés top-1-e, csak pontos címegyezésnél és nem egyértelműsítő lapnál → `wikipedia_url`. Cache a kérés kanonikus URL-je szerint (API-kulcs nélkül): minden válasz a site-adatbázisban, a találatok a `shared.duckdb`-ben; ismételt futásnál nincs újrahívás. Minden kérés a `validation_calls`-ban (kvóta, rate-limit, hiba). A 009-es migráció; a 010-es: `site.home_urls` (a kezdőoldalak, a site-profil írja; az entitás-réteg innen olvassa, nem a motorból).
